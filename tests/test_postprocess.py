...
class TestS3UploadFaultTolerance:
    ...
    def test_postprocess_script_uploads_fpm_trace_artifacts(self, tmp_path):
        """Producer-side FPM trace files remain part of the log upload."""
        mixin = self._create_mixin_with_runtime(tmp_path)
        script = mixin._build_postprocess_script("s3://test-bucket/run/", "")

        assert "fpm.sock" not in script
